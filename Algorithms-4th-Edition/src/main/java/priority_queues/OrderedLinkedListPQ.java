package priority_queues;

public class OrderedLinkedListPQ<T extends Comparable<T>> implements PriorityQueue<T> {

    // 定义链表节点
    private class Node {
        T item;
        Node next;

        Node(T item, Node next) {
            this.item = item;
            this.next = next;
        }
    }

    private Node first;  // 链表的第一个节点
    private int size;  // 优先队列的大小

    public OrderedLinkedListPQ() {
        first = null;
        size = 0;
    }

    @Override
    public void insert(T item) {
        if (first == null || item.compareTo(first.item) >= 0) {
            // 插入到链表的开始位置
            first = new Node(item, first);
        } else {
            // 找到正确的插入位置
            Node prev = first;
            Node current = first.next;
            while (current != null && item.compareTo(current.item) < 0) {
                prev = current;
                current = current.next;
            }
            prev.next = new Node(item, current);
        }
        size++;
    }

    @Override
    public T max() {
        if (isEmpty()) {
            throw new RuntimeException("Queue is empty");
        }
        return first.item;
    }


    @Override
    public T extractMax() {
        if (isEmpty()) {
            throw new RuntimeException("Queue is empty");
        }
        T maxItem = first.item;
        first = first.next;
        size--;
        return maxItem;
    }

    @Override
    public boolean isEmpty() {
        return size == 0;
    }

    @Override
    public int size() { return size; }

    // 以下接口未实现
    @Override public T extractMin() { return null; }

    @Override
    public T min() { return null; }

    @Override
    public void merge(PriorityQueue<T> other) { }

    @Override
    public void clear() {}

    public static void main(String[] args) {
        OrderedLinkedListPQ<Integer> pq = new OrderedLinkedListPQ<>();
        pq.insert(10);
        pq.insert(20);
        pq.insert(5);
        System.out.println(pq.extractMax());  // 输出: 20
        System.out.println(pq.max());  // 输出: 10
        System.out.println(pq.extractMax());  // 输出: 10
    }
}
